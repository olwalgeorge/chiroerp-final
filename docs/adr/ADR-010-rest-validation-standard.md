...
**Implementation Notes:** This ADR defines the enterprise validation standard and will be adopted incrementally as contexts implement APIs and shared validation utilities.
...
### 1. Direct Parameter Validation (Rejected)
**Approach**: Validate parameters directly in JAX-RS resource methods without BeanParam.

**Pros**:
- Less boilerplate code
- Simpler for single-parameter endpoints
- No DTO classes needed

**Cons**:
- Inconsistent error handling across endpoints
- `IllegalArgumentException` for UUID/date parsing → HTTP 500
- No cross-field validation support
- No centralized validation logic
- Scattered validation code (hard to audit)

**Why Rejected**: Insufficient for SAP-grade consistency; error handling varies by endpoint.

### 2. Spring Validator (Rejected)
**Approach**: Use Spring Framework's `@Validated` and `MethodValidationPostProcessor`.

**Pros**:
- Well-documented Spring ecosystem pattern
- Supports JSR-303/JSR-380 annotations
- AOP-based validation interception

**Cons**:
- Quarkus-specific extension required (breaks portability)
- Different exception types than Jakarta Validation
- Spring dependencies in non-Spring stack
- Performance overhead of Spring AOP

**Why Rejected**: Not Quarkus-native; adds unnecessary Spring dependencies.

### 3. Manual Validation Service (Rejected)
**Approach**: Create custom `ValidationService` class with manual validation logic.
...
**Cons**:
- Reinventing JSR-303/JSR-380 standards
- High maintenance burden
- No annotation-based validation
- Difficult to test and audit
- No integration with OpenAPI docs

**Why Rejected**: Reinventing the wheel; JSR-303 is industry-proven standard.

### 4. GraphQL with Custom Scalars (Considered for Future)
**Approach**: Use GraphQL schema with custom scalar types for validation.

**Pros**:
- Schema-first validation
- Strong typing at API contract level
- Client-side validation hints from schema
- Single request for multiple resources

**Cons**:
- Requires GraphQL infrastructure
- Different error format than REST
- Learning curve for team
- Not REST-compatible

**Decision**: **Deferred** - Consider for Phase 6+ if client demand justifies GraphQL adoption. Current REST-first strategy serves 95% of use cases.

### 5. API Gateway Validation (Complementary)
**Approach**: Perform validation at API Gateway before routing to services.

**Pros**:
- Early rejection of malformed requests
- Offload validation from service layer
- Consistent rate limiting and authentication

**Cons**:
- Gateway must know all validation rules (tight coupling)
- Duplicates domain validation logic
- Can't perform business logic validation
- Gateway becomes bottleneck

**Decision**: **Complementary** - Gateway performs basic validation (authentication, rate limiting, malformed JSON), services perform domain validation (business rules, cross-field checks). See ADR-004.
...
### Neutral
- Validation behavior will vary across contexts during the phased rollout.
...
- **ADR-020**: Event-Driven Architecture (validation of event payloads)
- **ADR-009**: Financial Accounting Domain (finance-specific validation rules)
- **ADR-012**: Tenant Customization Framework (custom field validation integration)
- **ADR-014**: Authorization Objects & SoD (authorization validation before business logic)
- **ADR-017**: Performance Standards & Monitoring (validation performance SLOs)
...
**Requirement**: Financial system changes must be auditable; validation failures must be logged.

**How We Comply**:
- **Audit Trail**: All validation failures on financial endpoints (`/api/v1/finance/*`) logged with user, tenant, request ID, rejected values
- **Immutable Logs**: Validation audit logs stored in append-only PostgreSQL tables with 7-year retention
- **Error Traceability**: Correlation IDs link validation failures to business transactions
- **Compliance Reports**: Quarterly validation failure reports for SOX auditors

### GDPR Compliance
**Requirement**: PII in validation errors must be minimized; users have right to access logs.

**How We Comply**:
- **Data Minimization**: Validation error messages exclude PII (e.g., "email format invalid" vs "john@example.com format invalid")
- **Purpose Limitation**: Validation logs used only for debugging/security; not for analytics
- **Right to Access**: Users can query their validation errors via `/api/v1/users/{userId}/validation-history`
- **Right to Erasure**: Validation logs purged on data subject request (after compliance retention period)

### OWASP API Security Top 10
**Requirement**: API validation must prevent security vulnerabilities.

**How We Comply**:
- **API4: Unrestricted Resource Consumption**: Rate limiting per tenant (100 req/min) prevents validation DOS
- **API8: Security Misconfiguration**: No stack traces in validation errors (production mode)
- **API10: Unsafe Consumption of APIs**: Input sanitization prevents injection attacks (SQL, XSS, command injection)

### PCI-DSS Compliance
**Requirement**: Payment card data validation must prevent data leakage.

**How We Comply**:
- **Requirement 3.4**: Card numbers validated with Luhn algorithm but never logged
- **Requirement 6.5.1**: Input validation on payment endpoints prevents injection attacks
- **Requirement 10.2**: All validation failures on payment endpoints logged for forensics

### Service Level Objectives
- **Validation Latency**: < 100ms (P95) including all validation rules
- **Validation Accuracy**: < 0.01% false positives (valid requests rejected)
- **Audit Log Reliability**: 100% of financial validation failures logged
