...
## Compliance
Enforce tenant isolation via layered controls:
- Application-level tenant scoping on all queries and commands.
- PostgreSQL RLS policies for row-level tenants.
- Security testing for cross-tenant access (automated + manual).
- Isolation tier change requires a compliance review and migration plan.
