...
**Core**
- Single-entity ledger, chart of accounts, journal entries, and period controls.
- Basic tax posting and audit trail.

**Advanced**
- Multi-entity and consolidation, multi-currency, intercompany eliminations.
- Regulatory reporting (GAAP/IFRS/SOX), bank reconciliation, advanced approvals.
...
**All features are in design/planning phase. No code has been implemented.**
...
- ⬜ AP/AR modules: Full implementation of accounts payable and receivable
- ⬜ financial-shared module: Money, Currency, FiscalPeriod implementations
- ⬜ Advanced features: Bank reconciliation, consolidation, tax engine, regulatory reporting

**Implementation Priorities:**
1. **Implement Metrics Annotations**: Add @Counted and @Timed to command handlers and REST endpoints
2. **Implement Health Checks**: Create FinanceHealthCheck implementing HealthCheck interface
3. **Implement Shared Financial Types**: Build proper Money, Currency value objects in financial-shared
4. **Create API Documentation**: Generate .rest files for Postman/IntelliJ HTTP Client testing
5. **Implement Audit Columns**: Add created_by/modified_by to all tables
6. **Build Load Tests**: Create k6/JMeter scripts for performance validation
7. **Design Chaos Tests**: Define chaos experiments for resilience testing

**Database Schema Design Goals:**
- Plan to use BIGINT for all monetary amounts (stores minor units: cents, pence, etc.) - prevents floating-point errors
- Include optimistic locking via version column on all mutable entities
- Add unique constraints on tenant + code combinations
- Add check constraint ensuring journal lines have either debit or credit (not both)
- Include foreign key cascades on journal_entry_lines for referential integrity
- Include audit columns (created_by, modified_by) from day one
- Design for future foreign currency columns (foreign_currency, foreign_amount, exchange_rate)
- Design for future dimension columns (cost_center_id, project_id, department_id)
...
### Phase 4: Financial Accounting Foundation (Months 1-6)

**Milestone 1: Core Ledger & Chart of Accounts** (Months 1-2)
- Database schema (`financial_accounting`) with Flyway migrations
- Ledger, ChartOfAccounts, Account aggregates with double-entry validation
- REST APIs: Ledger CRUD, Account hierarchy management
- Event publishing: `finance.ledger.created`, `finance.account.created`

**Milestone 2: Journal Entry & Posting** (Months 3-4)
- JournalEntry, PostingBatch aggregates with balanced debit/credit validation
- REST APIs: Journal entry creation, batch posting, reversal
- Accounting period validation (reject postings to closed periods)
- Event publishing: `finance.journal.posted`, `finance.journal.reversed`

**Milestone 3: Period Management & Close** (Months 5-6)
- AccountingPeriod aggregate with open/freeze/close workflow
- Period close validation (all batches posted, no pending approvals)
- REST APIs: Period lifecycle, year-end close
- Event publishing: `finance.period.frozen`, `finance.period.closed`

### Phase 5: Enterprise Enhancements (Months 7-12)

**Milestone 4: Multi-Currency & Exchange Rates** (Months 7-8)
- ExchangeRate aggregate with effective dating
- Currency translation (spot, average, closing rates)
- Unrealized gain/loss calculation
- REST APIs: Exchange rate management, currency conversion

**Milestone 5: Multi-Dimensional Accounting** (Months 9-10)
- CostCenter, ProfitCenter, Project dimensions
- Allocation rules for indirect costs
- Management reporting by dimension
- REST APIs: Dimension master data, allocation execution

**Milestone 6: Tax & Compliance** (Months 11-12)
- TaxCode aggregate with jurisdiction support
- VAT/GST/Sales Tax calculation engines
- Tax reporting (returns, summaries)
- REST APIs: Tax calculation, compliance reports

### Success Criteria
- **Performance**: Journal posting < 200ms (P95), period close for 1M transactions < 5 minutes
- **Data Integrity**: Zero double-entry violations, 100% balanced journal entries
- **Compliance**: SOX-compliant audit trail, GDPR-compliant retention
- **Integration**: 5+ upstream contexts integrated (AP, AR, Inventory, Payroll, Fixed Assets)
