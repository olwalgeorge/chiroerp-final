...
class UserOutboxJpaStore(
    ...
    override fun fetchPending(limit: Int, now: Instant): List<UserOutboxEntry> {
        val query = entityManager.createQuery(
            """
            SELECT o
            FROM UserOutboxJpaEntity o
            WHERE o.status = :pendingStatus
              AND o.nextAttemptAt <= :now
            ORDER BY o.createdAt ASC
            """.trimIndent(),
            UserOutboxJpaEntity::class.java,
        )
            .setParameter("pendingStatus", UserOutboxStatus.PENDING)
    ...
    override fun claimPending(limit: Int, now: Instant): List<UserOutboxEntry> {
    ...
    override fun markPublished(eventId: UUID, publishedAt: Instant) {
        entityManager.createQuery(
            ...
            .setParameter("publishedStatus", UserOutboxStatus.PUBLISHED)
    ...
    override fun markDead(eventId: UUID, attempts: Int, lastError: String?) {
        entityManager.createQuery(
            ...
            .setParameter("deadStatus", UserOutboxStatus.DEAD)
            .setParameter("attempts", attempts)
            .setParameter("lastError", lastError)
            .setParameter("eventId", eventId)
            .setParameter("pendingStatus", UserOutboxStatus.PENDING)
    ...
    override fun countPending(): Long = entityManager.createQuery(
        """
        SELECT COUNT(o)
        FROM UserOutboxJpaEntity o
        WHERE o.status = :pendingStatus
    ...
    override fun countDead(): Long = entityManager.createQuery(
        """
        SELECT COUNT(o)
        FROM UserOutboxJpaEntity o
    ...
    private fun toEntry(entity: UserOutboxJpaEntity): UserOutboxEntry = UserOutboxEntry(
