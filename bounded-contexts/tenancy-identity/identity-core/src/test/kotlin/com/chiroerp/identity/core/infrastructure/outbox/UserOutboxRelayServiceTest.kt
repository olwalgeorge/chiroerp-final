...
class UserOutboxRelayServiceTest {
    ...
    fun `relay marks event published on successful dispatch`() {
        ...
        assertThat(store.entries[entry.eventId]!!.status).isEqualTo(UserOutboxStatus.PUBLISHED)
        ...
        assertThat(meterRegistry.counter("chiroerp.identity.outbox.relay.dispatched").count()).isEqualTo(1.0)
        assertThat(meterRegistry.counter("chiroerp.identity.outbox.relay.failed").count()).isEqualTo(0.0)
    }
    ...
    fun `relay records failed attempt and retries with exponential backoff`() {
        ...
        val firstAttempt = Instant.parse("2026-02-11T12:00:00Z")
        ...
        assertThat(afterFailure.status).isEqualTo(UserOutboxStatus.PENDING)
        ...
        assertThat(afterSuccess.status).isEqualTo(UserOutboxStatus.PUBLISHED)
        assertThat(afterSuccess.publishedAt).isEqualTo(secondAttempt)
        assertThat(dispatcher.calls).isEqualTo(2)
        assertThat(meterRegistry.counter("chiroerp.identity.outbox.relay.failed").count()).isEqualTo(1.0)
        assertThat(meterRegistry.counter("chiroerp.identity.outbox.relay.dispatched").count()).isEqualTo(1.0)
    }
    ...
    fun `relay marks event dead after max attempts exceeded`() {
        ...
        assertThat(afterMaxAttempts.status).isEqualTo(UserOutboxStatus.DEAD)
        assertThat(afterMaxAttempts.lastError).isEqualTo("Intentional failure")
        assertThat(dispatcher.calls).isEqualTo(maxAttempts)
        assertThat(meterRegistry.counter("chiroerp.identity.outbox.relay.dead").count()).isEqualTo(1.0)
        assertThat(meterRegistry.counter("chiroerp.identity.outbox.relay.failed").count())
            .isEqualTo(maxAttempts.toDouble())
    }
    ...
    fun `dead entries are not fetched by claimPending`() {
        ...
        ).copy(status = UserOutboxStatus.DEAD)
    ...
    private fun sampleEntry(
    ...
    ): UserOutboxEntry = UserOutboxEntry(
        eventId = eventId,
        ...
        payload = """{"eventType":"UserCreated"}""",
        occurredAt = createdAt,
        createdAt = createdAt,
        status = UserOutboxStatus.PENDING,
        nextAttemptAt = nextAttemptAt,
    )

    private class InMemoryOutboxStore : UserOutboxStore {
        val entries = linkedMapOf<UUID, UserOutboxEntry>()
        ...
        override fun fetchPending(limit: Int, now: Instant): List<UserOutboxEntry> = claimPending(limit, now)

        override fun claimPending(limit: Int, now: Instant): List<UserOutboxEntry> = entries.values
            .filter { it.status == UserOutboxStatus.PENDING && !it.nextAttemptAt.isAfter(now) }
        ...
        override fun markPublished(eventId: UUID, publishedAt: Instant) {
            ...
            entries[eventId] = existing.copy(
                status = UserOutboxStatus.PUBLISHED,
        ...
        override fun markDead(eventId: UUID, attempts: Int, lastError: String?) {
            val existing = entries[eventId] ?: return
            entries[eventId] = existing.copy(
                status = UserOutboxStatus.DEAD,
                publishAttempts = attempts,
                lastError = lastError,
            )
        }

        override fun countPending(): Long = entries.values.count { it.status == UserOutboxStatus.PENDING }.toLong()

        override fun countDead(): Long = entries.values.count { it.status == UserOutboxStatus.DEAD }.toLong()
    }

    private class RecordingDispatcher : UserOutboxDispatcher {
        val dispatched = mutableListOf<UserOutboxEntry>()
    ...
    private class FailingOnceDispatcher : UserOutboxDispatcher {
        var calls: Int = 0
    ...
    private class AlwaysFailingDispatcher : UserOutboxDispatcher {
        var calls: Int = 0
